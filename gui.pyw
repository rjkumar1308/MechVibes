...
def launchgui(volume):
    app = wx.App()
    frame = MyFrame(volume)
    app.MainLoop()
    
def launchroot(volume):
    root.main(volume)
...
if __name__ == '__main__':
    ...
    fo.close()
    volume = Volume(initail_volume)

    p2 = threading.Thread(target=launchroot, args=(volume, ), daemon=True)
    p1 = threading.Thread(target=launchgui, args=(volume, ), daemon=True)
